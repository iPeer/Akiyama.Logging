...
namespace Akiyama.Logging.Configuration
{
    public class LoggerConfig
    {
        ...
        public bool DebugOutputDisabled { get; set; } = RuntimeInformation.FrameworkDescription.StartsWith(".NET Framework");
        ...
        public LoggerConfig(string name, LoggerType type = LoggerType.CACHED, LogLevel level = LogLevel.INFO,
            string lineFormat = "[<time>] [<level>] <name>: <message>", LogFormatter formatter = LogFormatter.DEFAULT, string dateTimeFormat = @"yyyy-MM-dd HH\:mm\:ss",
            string outputDirectory = "./logs/", int cacheSize = 30, int cacheBypassLength = 5000, int maxCycledFiles = 5, bool consoleOutputDisabled = false,
            bool debugOutputDisabled = false)
        {
            this.Name = name;
            this.Type = type;
